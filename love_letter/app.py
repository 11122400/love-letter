...
<style>
    ...
    .heart-svg { width: 150px; height: 150px; display: block; margin: 0 auto; }
    .heart-poem { text-align: center; font-size: 11px; color: #e91e63; margin-top: 5px; font-weight: bold; }
    .heart-date { text-align: center; font-size: 10px; color: #999; font-style: italic; }
...
def show_main(letter):
    ...
    photos = [
        {"poem": "那日人间风恰好\n辛得世间有你存", "file": "photo1.jpg", "label": "💕 初见"},
        {"poem": "容颜恰是人间绝\n唯憾未能伴身侧", "file": "photo2.jpg", "label": "🌟 笑容"},
        {"poem": "仗剑同迎千万难\n护卿岁岁无波澜", "file": "photo3.jpg", "label": "🎉 写真"},
        {"poem": "揽卿入怀\n愿卿无忧", "file": "photo4.jpg", "label": "💝 幸福"},
    ]
    
    videos = [
        {"poem": "共踏山巅云尽处\n长相厮守伴余生", "file": "video1.mp4", "label": "💕 第一次爬雪山"},
        {"poem": "每一个平凡的日子\n都因为你而特别", "file": "video2.mp4", "label": "🌟 你觉得可爱的"},
    ]
    ...
    for i, p in enumerate(photos):
        with cols[i]:
            if st.button(p["poem"], key="photo_" + str(i), use_container_width=True):
    ...
    for i, v in enumerate(videos):
        with cols2[i]:
            if st.button(v["poem"], key="video_" + str(i), use_container_width=True):
                st.session_state.floating_media = {
...
def show_floating_media():
    ...
    if media_type == "image":
        img = safe_load_media(file, "image")
        if img:
        ...
        video_bytes = safe_load_media(file, "video")
        if video_bytes:
